...
- Scene card (Play Setup screen) — top right, one set of numbers per scene, for your lines in
  that scene only. Shown in Run mode; in Edit mode the same corner holds the edit/delete icons
  instead.
- Play card (Play List screen) — bottom right, Total/Hidden/Ratio added up across every scene
  in the play (no Sections row here). Shown in both Edit and Run mode.

> **Tip:** A card with no lines of your own yet — a scene not assigned to you, or a play with
> nothing added — simply shows no stats block.
